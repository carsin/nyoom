...
const fetchConversations = async () => {
  ...
  try {
    unsubscribeConvoListener = await chatService.fetchConversations(
      conversations,
      userCache
    );
  } catch (error: any) {
    toast.value = {
      isOpen: true,
      message: "Error fetching conversation list: " + error.message,
      color: "danger",
    };
  } finally {
...
const prepareConversation = async (conversationOrUser) => {
  isLoading.value = true;
  try {
    const result = await chatService.prepareConversation(
      conversationOrUser,
      activeConversation,
      messages
    );
...
const sendMessage = async () => {
  if (isLoading.value) {
    toast.value = {
      isOpen: true,
      message: "Already sending message!",
      color: "danger",
    };
    return;
  }
  ...
  try {
    ...
    if (!result.success) {
      ...
      unsubscribeMessageListener = chatService.listenToMessages(
...
function navigateToUserProfile(message) {
  const username =
    message.senderId === currentUser?.uid
      ...
      : activeConversationDisplay.value.username;
